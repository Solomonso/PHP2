...
<?php
    	...
    	if(!$conn)
    	{
    		echo "Connection link not established ".mysqli_error($conn);

    	}else{echo "<h1>Connection to the server established</h1>";}	
    		...
    		{	
    			...
    			$sql_email = "SELECT user_email FROM stenden_user WHERE user_email = ? LIMIT 1";
    				...
    				if($stmt_name = mysqli_prepare($conn,$sql_name))
    				{
    					...
    					}else{echo "Unable ".mysqli_error($conn);}
    							...
    							&& ($_FILES["image"]["size"] < 600000) )
    						{
    							...
    							else
    							{
    							   	...
    							   	if(mysqli_stmt_execute($stmt))
    								{
    									...
    									echo "<a href=login.php class='text'><button>Click Here to Login</button></a>";
    								
    								}else{echo "Unable to Create acoount ".mysqli_error($conn);}
    	...
    	<div class="form-div-signup">
    			<div class="form-style-signup">
    				<form action="sign_up.php" method="POST" enctype="multipart/form-data">
    					...
    					<br>
    					<p class="form-field"><input type="text" name="user_name" size="30" id="name-field" class="style" placeholder="Username"></p>
    					<p class="form-field"><input type="password" name="password" size="30" id="pass-field" class="style" required placeholder="Password"></p>
    					<p class="form-field"><input type="email" name="email" size="30" id="email-field" class="style" required placeholder="Email"></p>
